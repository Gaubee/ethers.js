...
export class BrainWallet extends ethers.Wallet {
    static _generate(
        username: ethers.Bytes | string,
        password: ethers.Bytes | string,
        legacy: boolean,
        progressCallback?: ethers.utils.ProgressCallback,
    ): Promise<BrainWallet> {
        if (!warned) {
            logger.warn(
                "Warning: using Brain Wallets should be considered insecure (this warning will not be repeated)",
            );
            warned = true;
        }
        ...
        return scrypt
            .scrypt(passwordBytes, usernameBytes, 1 << 18, 8, 1, 32, progressCallback)
            .then((key: Uint8Array) => {
                if (legacy) {
