...
describe("Test HD Mnemonic Phrases", function testMnemonic() {
    ...
    tests.forEach(function (test) {
        ...
        it("converts mnemonic phrases - " + test.name, function () {
            ...
            assert.equal(
                ethers.utils.mnemonicToSeed(test.mnemonic, test.password),
                test.seed,
            ...
            if (test.locale === "en") {
                assert.equal(
                    ethers.utils.entropyToMnemonic(test.entropy),
                    test.mnemonic,
                    "Converts entropy to mnemonic " + test.name + " (default en)",
                );

                assert.equal(
                    ethers.utils.mnemonicToEntropy(test.mnemonic),
                    test.entropy,
            ...
            assert.equal(
                ...
                test.mnemonic.normalize("NFKD"),
                "Converts entropy to mnemonic " + test.name,
            );

            assert.equal(
                ethers.utils.mnemonicToEntropy(test.mnemonic, wordlist),
                test.entropy,
...
describe("HD error cases", function () {
    const testInvalid = ["", "m/45/m", "m/44/foobar"];
