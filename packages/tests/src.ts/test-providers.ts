...
Object.keys(blockchainData).forEach((network) => {
    ...
    function addObjectTest(
        name: string,
        func: (provider: ethers.providers.Provider) => Promise<any>,
        expected: any,
        checkSkip?: CheckSkipFunc,
    ) {
        testFunctions.push({
    ...
    tests.blocks.forEach((test) => {
        addObjectTest(
            ...
            test,
            (provider: string, network: string, test: TestDescription) => {
    ...
    tests.transactionReceipts.forEach((test) => {
        ...
        addObjectTest(
            `fetches transaction receipt ${hash}`,
            async (provider: ethers.providers.Provider) => {
    ...
    */
    ...
    addErrorTest(ethers.utils.Logger.errors.NONCE_EXPIRED, async (provider: ethers.providers.Provider) => {
        return provider.sendTransaction(
            "0x02f86e05808459682f008459682f14830186a09475544911a6f2e69ceea374f3f7e5ea9c987ece098304cb2f80c001a0d9585a780dde9e7d8c855aacec0564054b49114931fd7e320e4e983009d864f7a050bee916f2770ef17367256d8bccfbc49885467a6ba27cf5cc57e8553c73a191",
        );
    });
                    ...
                    for (let attempt = 0; attempt < attempts; attempt++) {
                        try {
                            const result = await Promise.race([
                                ...
                                waiter(timeout * 1000).then((result) => {
    ...
    [
        {
            title: "ERC-1155",
            name: "nick.eth",
            value: "https://lh3.googleusercontent.com/hKHZTZSTmcznonu8I6xcVZio1IF76fq0XmcxnvUykC-FGuVJ75UPdLDlKJsfgVXH9wOSmkyHw0C39VAYtsGyxT7WNybjQ6s3fM3macE",
        },
...
describe("Test EIP-2544 ENS wildcards", function () {
    ...
    it("Resolves recursively", async function () {
        ...
        assert.equal(
            (await resolver.getAvatar()).url,
            "https://static.ricmoo.com/uploads/profile-06cb9c3031c9.jpg",
            "gets passed-through avatar",
        );
        assert.equal(
            await resolver.getAddress(),
            "0x4B711A377B1b3534749FBe5e59Bcf7F94d92EA98",
