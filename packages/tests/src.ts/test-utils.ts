...
describe("Test Unit Conversion", function () {
    ...
    tests.forEach((test) => {
        ...
        it("parses " + test.ether + " ether", function () {
            assert.ok(
                ethers.utils.parseEther(test.ether.replace(/,/g, "")).eq(wei),
        ...
        ["kwei", "mwei", "gwei", "szabo", "finney", "satoshi"].forEach((name: UnitName) => {
            ...
            if (test[name]) {
                it("parses " + test[name] + " " + name, function () {
                    this.timeout(120000);
                    assert.ok(
                        ethers.utils.parseUnits(test[name].replace(/,/g, ""), unitName).eq(wei),
...
describe("Test Namehash", function () {
    ...
    const goodNames = ["ricmoo.eth", "foo", "foo.bar"];

    const badNames = [".", "..", "ricmoo..eth", "ricmoo...eth", ".foo", "foo."];
...
describe("Test Base64 coder", function () {
    ...
    it("encodes and decodes the example from wikipedia", function () {
        ...
        assert.equal(
            ethers.utils.toUtf8String(ethers.utils.base64.decode(encoded)),
            decodedText,
...
describe("Test UTF-8 coder", function () {
    ...
    let BadUTF = [
        ...
        {
            bytes: [0xf0, 0x82, 0x82, 0xac],
            reason: overlong,
            ignored: "",
            replaced: "\u20ac",
            name: "wikipedia overlong encoded Euro sign",
        },
        ...
        // Reserved UTF-16 Surrogate halves
        {
            ...
            reason: utf16Surrogate,
            ignored: "",
            replaced: "\ufffd",
            name: "utf-16 surrogate - U+d800",
        },
        {
        ...
        {
            bytes: [0xc2, 0x00],
            reason: missingContinue,
            ignored: "\u0000",
            replaced: "\ufffd\u0000",
            name: "invalid continuation byte - 0xc200",
        },
        {
            bytes: [0xc2, 0x40],
            reason: missingContinue,
            ignored: "@",
            replaced: "\ufffd@",
            name: "invalid continuation byte - 0xc240",
        },
        {
            bytes: [0xc2, 0xc0],
            reason: missingContinue,
            ignored: "",
            replaced: "\ufffd\ufffd",
            name: "invalid continuation byte - 0xc2c0",
        },
...
describe("Test Bytes32String coder", function () {
    ...
    it("encodes an ens name", function () {
        ...
        assert.equal(
            bytes32,
            "0x7269636d6f6f2e66697265666c792e6574680000000000000000000000000000",
            "formatted correctly",
        );
...
describe("Test Signature Manipulation", function () {
    ...
    tests.forEach((test) => {
        it("autofills partial signatures - " + test.name, function () {
            ...
            const s = data.pop(),
                r = data.pop(),
                v = parseInt(data.pop().substring(2), 16);
...
describe("Test Typed Transactions", function () {
    ...
    function makeEqualsArray(
        equals: (name: string, a: any, b: any) => boolean,
    ): (name: string, a: any, b: any) => boolean {
    ...
    function equalsAccessList(name: string, a: Array<any>, b: Array<any>): boolean {
        return (
            ...
            equalsArray(
                `${name}-storageKeys`,
                a.map((f) => f.storageKeys),
                b.map((f) => f.storageKeys),
                makeEqualsArray(equalsData),
            )
        );
    }
    ...
    function equalsCommonTransaction(name: string, a: any, b: any): boolean {
        return (
            equalsNumber(`${name}-type`, a.type, b.type, 0) &&
    ...
    function equalsEip1559Transaction(name: string, a: any, b: any): boolean {
        return (
            equalsNumber(`${name}-maxPriorityFeePerGas`, a.maxPriorityFeePerGas, b.maxPriorityFeePerGas, 0) &&
    ...
    function equalsEip2930Transaction(name: string, a: any, b: any): boolean {
        return equalsNumber(`${name}-gasPrice`, a.gasPrice, b.gasPrice, 0) && equalsCommonTransaction(name, a, b);
    }
...
describe("BigNumber", function () {
    ...
    [
        ...
        },
        {
            ...
            expected: "0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
        },
        {
            value: "-0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
            expected: "0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
...
describe("EIP-712", function () {
    ...
    tests.forEach((test) => {
        it(`encoding ${test.name}`, function () {
            ...
            assert.equal(
                ethers.utils._TypedDataEncoder.hash(test.domain, test.types, test.data),
                test.digest,
                "digest",
            );
        });
    ...
    const Tests = [
        {
            ...
            preimage:
                "0x01f839030d8465cf89a0825b689432162f3581e88a5f62e8a61892b42c46e2c18f7b8080d7d6940000000000000000000000000000000000000000c0",
            tx: {
                hash: "0x48bff7b0e603200118a672f7c622ab7d555a28f98938edb8318803eed7ea7395",
                type: 1,
                chainId: 3,
