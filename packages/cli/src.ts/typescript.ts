...
export function generate(contract: ContractCode, bytecode?: string): string {
    ...
    lines.push(
        `        return new (<{ new(...args: any[]): ${contract.name} }>(this.constructor))(this.address, providerOrSigner)`,
    );
    lines.push("    }");
    lines.push("");
    lines.push(`    attach(addressOrName: string): ${contract.name} {`);
    lines.push(
        `        return new (<{ new(...args: any[]): ${contract.name} }>(this.constructor))(addressOrName, this.signer || this.provider)`,
    );
    lines.push("    }");
        ...
        if (fragment.constant == false) {
            ...
            if (fragment.outputs.length === 1) {
                ...
                if (fragment.outputs.filter((o) => !!o.name).length === fragment.outputs.length) {
                    output =
                        "Promise<{ " +
                        fragment.outputs.map((o, i) => (o.name || "arg" + String(i)) + ": " + getType(o)).join(", ") +
                        " }>";
                } else {
    ...
    if (bytecode == null) {
        lines.push(
            '        return ethers.errors.throwError("no bytecode provided during generation", ethers.errors.UNSUPPORTED_OPERATION, { operation: "contract.bytecode" });',
        );
    } else {
